...
ActiveRecord::Base.transaction do
  ...
  list_double = []
  test3.each_with_index do |t, _index|
    next unless test3.select do |test|
      test.user_id == t.user_id && test.activity_ref_id == t.activity_ref_id && test.season_id == t.season_id
    end.length > 1
  ...
  # delete
  list_uniq.each do |t|
    double = list_uniq.select do |test|
      test.user_id == t.user_id && test.activity_ref_id == t.activity_ref_id && test.season_id == t.season_id
    end
    ...
    next unless double.length > 1 && !test
