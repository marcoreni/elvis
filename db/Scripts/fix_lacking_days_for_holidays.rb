...
dates = [DateTime.parse("2021-12-17"), DateTime.parse("2021-12-18"), DateTime.parse("2022-02-04"),
         DateTime.parse("2022-02-05"), DateTime.parse("2022-04-08"), DateTime.parse("2022-04-09")]

Activity.transaction do
  dates.each do |date|
    activities = Activity.includes(:time_interval).joins(:time_interval).where({ time_intervals: { start: (s.start..s.end) } }).where(
      "extract(dow from time_intervals.start) = extract(dow from ?::date)", date
    )

    activities.each do |activity|
      if ActivityInstance.joins(:time_interval).where(activity: activity).where("time_intervals.start::date = ?::date",
                                                                                date).any?
        next
      end
