...
# la liste des ids doit être récupéré lors du script des creates
activities = Activity.where(id: [1343, 1345, 1442, 1346, 1385, 1386, 1349, 1352, 1356, 1316, 1358, 1353, 1354, 1355,
...
s = Season.find 5
holidays_dates = s.holidays.map { |h| h.date.to_date }
...
Activity.includes(:time_interval, teachers_activities: { teacher: { planning: :time_intervals } },
  ...
  instances = instances.joins(:time_interval).where("time_intervals.start::date IN (?)", holidays_dates)

  # a.teacher.planning.time_intervals.destroy instances.map(&:time_interval).compact
  # a.students.map(&:user).compact.map(&:planning).compact.each { |p| p.time_intervals.destroy instances.map(&:time_interval).compact }

  intervals_ids = instances.map(&:time_interval).compact.map(&:id)
  instances.update_all(time_interval_id: nil)
  TimeInterval.where(id: intervals_ids).destroy_all
