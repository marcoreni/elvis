...
CSV.read("levels.csv", headers: false, col_sep: ";").each do |row|
  ...
  if !u.nil? && !row[3].nil?
    evaluation_level_ref = EvaluationLevelRef.where(
      "translate(trim(label), '-éàçäëïöüâêîôû''', ' eacaeiouaeiou') ILIKE translate(?, '-éàçäëïöüâêîôû''', ' eacaeiouaeiou')", "%#{row[3].strip}%"
    ).first
    ...
    unless level.nil?
      ...
      new_level.can_continue = case row[4]
                               when "O"
                                 false
                               when "N"
                                 true
