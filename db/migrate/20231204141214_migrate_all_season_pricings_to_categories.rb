  ...
  def up
    ...
    pricings.each do |pricing|
      ...
      ActiveRecord::Base.connection.execute("SELECT * FROM activity_ref_season_pricings WHERE pricing_id = #{pricing['id']}").each do |season_pricing|
        ...
        arp = ActivityRefPricing.new(activity_ref_id: season_pricing["activity_ref_id"],
                                     price: season_pricing["price"], pricing_category_id: pc.id, from_season_id: season_pricing["season_id"], to_season_id: season_pricing["season_id"])
