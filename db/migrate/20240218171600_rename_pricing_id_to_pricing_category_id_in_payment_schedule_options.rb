class RenamePricingIdToPricingCategoryIdInPaymentScheduleOptions < ActiveRecord::Migration[6.1]
  def up
    ...
    if column_exists?(:payment_schedule_options, :pricing_id)
      ...
      if table_exists?(:pricings)
        execute("insert into pricing_categories (name, number_lessons, is_a_pack, created_at, updated_at) select label as name, 31 as number_lessons, false as is_a_pack, now() as created_at, now() as updated_at from pricings order by id asc")
      end
  ...
  def down
    ...
    add_foreign_key :payment_schedule_options, :pricings, column: :pricing_id if foreign_key_exists?(
      :payment_schedule_options, :pricing_categories
    )
  end
end
