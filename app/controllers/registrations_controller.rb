...
class RegistrationsController < Devise::RegistrationsController
  ...
  def create
    verify_recaptcha(action: "sign_up")
    ...
    if score < min_score
      ...
      begin
        if !Rails.env.development? && Sentry.initialized?
          Sentry.capture_message("Recaptcha score too low for user #{resource.email} (#{score}/#{min_score})")
        end
        Rails.logger.error("Recaptcha score too low for user #{resource.email} (#{score}/#{min_score})")
  ...
  def validate_user_fields(record, request)
    ...
    record.unique_entry
    validated && record.errors.details.empty?
  end
