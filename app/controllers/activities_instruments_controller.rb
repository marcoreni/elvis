class ActivitiesInstrumentsController < ApplicationController
  ...
  def add_option
    ...
    if student_already_assigned_in_activity?(@desired_activity.activity_application.user.id, @activities_instrument)
      render json: { error: "Cet élève est déjà assigné à un autre rôle dans cet atelier" },
    ...
    Activity.transaction do
      ...
      @activities_instrument.update!(user: @desired_activity.activity_application.user, is_validated: false,
                                     attempt_date: attempt_date)
    end

    render json: Utils.format_for_suggestion(@desired_activity.activity_application.user,
                                             @activities_instrument.activity, @desired_activity.activity_application.begin_at)
  end
  ...
  def add_student
    ...
    @activities_instrument.update!(user: @desired_activity.activity_application.user, is_validated: true,
                                   attempt_date: nil)
