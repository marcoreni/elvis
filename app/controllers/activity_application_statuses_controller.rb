class ActivityApplicationStatusesController < ApplicationController
  ...
  before_action -> { render status: 401, json: { error: "forbidden" } and return }, if: lambda {
    ActivityApplicationStatus::BUILTIN_IDS.include?(params[:id].to_i)
  ...
  def create
    ...
    ActivityApplicationStatus.create(activity_application_status_params)
    ...
    respond_to do |format|
      format.json do
        if linked_applications.any?
          render json: t("controllers.activity_application_statuses.destroy.linked_applications", count: linked_applications.count),
      ...
      format.html do
        if linked_applications.any?
          flash[:error] =
            t("controllers.activity_application_statuses.destroy.linked_applications",
              count: linked_applications.count)
        else
          status.destroy
