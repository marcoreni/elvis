...
class FailedPaymentImportsController < ApplicationController
  ...
  def import_single
    ...
    head :not_found, details: "import" if failed.nil?
    ...
    Date.strptime(params[:cashing_date], le_french_format)
    ...
    if payer.nil?
      ...
      render json: { success: false, message: "Le.a payeur.se n'a pas été trouvé.e",
                     failed_payment_import: failed }
      return
    end

    if payer.payment_schedule.nil?
      failed.update(failed_payment_import_reason: payer_without_schedule, user_id: payer.id,
                    first_name: first_name, last_name: last_name)
      render json: { success: false, message: "Le.a payeur.se n'a pas d'échéancier",
                     failed_payment_import: failed }
      return
    end
    ...
    if due_payment_candidates.nil? || due_payment_candidates.empty?
      failed.update(failed_payment_import_reason: due_not_found, user_id: payer.id, first_name: first_name,
                    last_name: last_name)
    ...
    if due_payments.empty?
      failed.update(failed_payment_import_reason: different_amounts, user_id: payer.id, first_name: first_name,
                    last_name: last_name)
  ...
  def create_reasons
    FailedPaymentImportReason.find_or_create_by(code: "payer_not_found",
                                                label: "Payeur introuvable")
    FailedPaymentImportReason.find_or_create_by(code: "payer_without_schedule",
                                                label: "Payeur sans échéancier")
    FailedPaymentImportReason.find_or_create_by(code: "due_not_found",
                                                label: "Échéance introuvable")
    FailedPaymentImportReason.find_or_create_by(code: "different_amounts",
                                                label: "Montants différents")
    render html: helpers.tag.h1("Insertions réussies")
  end

  def bulkdelete
    @to_delete = if params[:all]
                   FailedPaymentImport.all
                 else
                   FailedPaymentImport.where(id: params[:targets])
                 end
  ...
  def migrate_reasons
    ...
    due_not_found = FailedPaymentImportReason.find_or_create_by(code: "due_not_found",
                                                                label: "Échéance introuvable")
    payment_found = FailedPaymentImportReason.find_or_create_by(code: "payment_found",
                                                                label: "Paiement déjà existant")
