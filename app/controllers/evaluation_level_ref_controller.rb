...
class EvaluationLevelRefController < ApplicationController
  ...
  def temp_to_new_level_management_system
    ...
    applications.each do |application|
      next if application.user.evaluation_level_ref_id.nil?

      evaluation_level_ref_id = application.user.evaluation_level_ref_id
      user_id = application.user.id

      user_activity_ref_ids = []
      application.desired_activities.each do |da|
        user_activity_ref_ids.push(da.activity_ref_id) unless user_activity_ref_ids.include?(da.activity_ref_id)
      ...
      user_activity_ref_ids.each do |activity_ref_id|
        Level.create(activity_ref_id: activity_ref_id, evaluation_level_ref_id: evaluation_level_ref_id,
                     user_id: user_id)
      end
    end
  end
