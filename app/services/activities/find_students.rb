module Activities
  class FindStudents
    ...
    def execute
      ...
      activity_students = activities.each_with_object({}) do |activity, h|
        h[activity.id] = activity.users
      end
      activity_options = activities.each_with_object({}) do |activity, h|
      ...
      applications = ActivityApplication
                     ...
                     .where({ user: activity_students.values.flatten,
                              desired_activities: { activity_id: activity_ids } })
                     .index_by(&:user_id)
      ...
      activity_students.each do |activity_id, students|
        ...
        # Inactives
        result[activity_id][:inactive] = students.select do |s|
          ...
          is_inactive
        end.as_json(level_includes)

        # Actives
        result[activity_id][:active] = (students - result[activity_id][:inactive]).as_json(level_includes)

        # Options
        result[activity_id][:options] =
          (activity_options[activity_id].as_json(level_includes) - result[activity_id][:active]).as_json(level_includes)
      end
