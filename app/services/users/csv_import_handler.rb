module Users
  ...
  class CsvImportHandler
    ...
    MANDATORY_HEADERS = ["Prenom", "Nom", "Date de naissance", "Email", "Adresse", "Code postal", "Ville", "Telephone",
                         "Role"].freeze
    ...
    def addMessageReport(key, message, line)
      ...
      return unless @import_report[:errors][key][:message].empty?
    ...
    def analyse_field(header, field, _line)
      if field.nil?
        if MANDATORY_VALUES.any? header
          addErrorRow("Une ou plusieurs valeurs obligatoires sont manquantes",
                      "Les valeurs obligatoires sont celles du nom, du prénom, de la date de naissance et de l'adresse mail de votre utilisateur.")
          return
        else
          field = ""
        end
      ...
      if header == "Telephone" && field.present? && Phonelib.invalid?(field)
        addErrorRow("Un ou plusieurs numéros de téléphone sont invalides",
                    "Veuillez vérifier les numéros de téléphone concernés.")
      end

      if header == "Date de naissance" && field.present?
        field = field.gsub("/", "-")
        if !valid_date?(field)
          addErrorRow("Une ou plusieurs dates de naissance sont invalides",
                      "Une date de naissance valide suit le format suivant: jour-mois-année.")
        elsif Date.parse(field) > Date.today
          addErrorRow("Une ou plusieurs dates de naissance sont invalides",
                      "Une date de naissance valide doit forcément précéder la date actuelle.")
        end
      end

      return unless header == "Role"
      return unless field != "administrateur" && field != "professeur" && field.present?

      addErrorRow("Un ou plusieurs rôles sont invalides",
                  "Le rôle renseigné est invalide. Les options sont soit 'administrateur', soit 'professeur', dans tout autre cas laissez le champ vide.")
    end
    ...
    def import_users(file)
      ...
      if headers.sort != MANDATORY_HEADERS.sort.map { |h| h.capitalize }
        addMessageReport("Un ou plusieurs en-têtes sont manquant(s) ou invalide(s)",
                         "Les en-têtes doivent être: Prenom, Nom, Date de naissance, Email, Adresse, Code postal, Ville, Telephone, Role.", 0)
