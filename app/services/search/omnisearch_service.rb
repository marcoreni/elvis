...
module Search
  ...
  class OmnisearchService
    ...
    def initialize(search_value)
      @words = search_value.to_s.strip.split(/\s+/).map { |w| w.gsub(/[^\p{L}\p{N}]/, "") }.reject(&:blank?).first(10)
    end

    def call
      return { results: [], total: 0 } if @words.empty?

      matches = SOURCE_METHODS.flat_map { |method| send(method) }
      { results: matches.first(RESULT_LIMIT).map { |attributes| { attributes: attributes } }, total: matches.size }
    end
