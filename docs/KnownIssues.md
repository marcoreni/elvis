...
## `spec/controllers/application_controller_spec.rb` / `DeviseMailer` order-dependent flake — still open

**Status: unresolved, timing-dependent, not purely example-order-dependent.** First surfaced
2026-09-12 (a new, unrelated spec file's mere presence in the load list flips this — confirmed twice
with two different new spec files). `bundle exec rspec --order random --seed 4` (full suite) mostly
but not always reproduces the same failure at the same seed with identical code, and adding any
diagnostic instrumentation (a `puts`, an `ActiveSupport::Notifications.subscribe`) measurably changes
the failure rate — a genuine timing/scheduling Heisenbug, not a simple ordering bug. A second,
differently-shaped flake (`spec/requests/locations_destroy_spec.rb` expecting a French `flash[:error]`
...
events for either `devise/mailer/confirmation_instructions.html.erb` or `layouts/mailer.html.erb`,
yet the returned body is byte-for-byte the layout's own markup with a blank `yield` — points at
something in the ActionMailer/ActionView render-and-cache path returning already-produced content,
not at a locale-variable-holds-the-wrong-value story. Whoever picks this up next should start there.

Ruled out already (don't re-investigate): no direct `I18n.locale =`/`default_locale =` assignment or
`stub_const("Elvis::SUPPORTED_LOCALES", ...)` anywhere in the tree; `I18n.with_locale` in the
installed gem is a standard `ensure`-protected implementation; `I18n::Config`'s Fiber-based storage
doesn't cross-contaminate between threads; forcing `BaseEventJob.queue_adapter = :inline` (so
listener jobs run synchronously in tests) does not stop the flip from reproducing. A related, real
bug was found and fixed while chasing this (`Season.current`/`current_apps_season` had no cache
invalidation, unlike `Parameter` — see `app/models/season.rb`'s `after_commit :expire_season_caches`
and its spec) but confirmed **not** the mechanism here, since mailer specs never touch
`ApplicationController`/`Ability`/`Season`.

Prefer low-overhead instrumentation (a tightly-filtered `TracePoint`, post-hoc `ObjectSpace`/GC
stats) over `puts`/notification subscribers when investigating further — the latter measurably
perturbs the timing enough to stop the repro.
