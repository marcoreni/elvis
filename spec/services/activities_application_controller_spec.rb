...
RSpec.describe ActivityApplications::TesImportHandler, type: :service do
  describe "#guess_activity_ref_and_level" do
    before do
      ...
      FactoryBot.create(:activity_ref, label: "Guitare - cours individuel - 30 minutes",
                                       activity_ref_kind: ActivityRefKind.find_by(name: "Guitare"))
      FactoryBot.create(:activity_ref, label: "Guitare - cours duo",
                                       activity_ref_kind: ActivityRefKind.find_by(name: "Guitare"))
      FactoryBot.create(:activity_ref, label: "Guitare - cours collectif",
                                       activity_ref_kind: ActivityRefKind.find_by(name: "Guitare"))

      FactoryBot.create(:activity_ref_kind, name: "Piano / Clavier")
      FactoryBot.create(:activity_ref, label: "Piano / Clavier - cours individuel - 30 minutes",
                                       activity_ref_kind: ActivityRefKind.find_by(name: "Piano / Clavier"))
      FactoryBot.create(:activity_ref, label: "Piano / Clavier - cours duo",
                                       activity_ref_kind: ActivityRefKind.find_by(name: "Piano / Clavier"))
      FactoryBot.create(:activity_ref, label: "Piano / Clavier - cours collectif",
                                       activity_ref_kind: ActivityRefKind.find_by(name: "Piano / Clavier"))
    ...
    context "when instrument contains level" do
      ...
      context "when activite is atelier" do
        ...
        it "returns the correct activity reference" do
          ...
          expect(result).to eq([{ activity_ref_id: ar.id, instrument_names: "Guitare", is_workshop: true,
                                  level: "2 ans" }])
        end
      end
    end
