...
vi.mock("../../../tools/api", () => ({
    ...
    get: (...args) => {
        apiState.lastGetArgs = args;
        return Promise.resolve(apiState.getResolve);
    },
}));
...
describe("parameters lot-E — i18n layer", () => {
    ...
    test("regression: the lot-A plannings.tabs.* / evaluations.tabs.* keys survive", () => {
        expect(tP("fr")("plannings.tabs.schoolAvailability")).toBe(
            "Disponibilité de l'école"
        );
        expect(tP("en")("plannings.tabs.schoolAvailability")).toBe(
            "School availability"
        );
    ...
    test.each(["fr", "en"])(
        ...
        (lng) => {
            const v = tP(lng)("evaluations.levels.deleteConfirm", {
                name: "Zephyr",
            });
    ...
    test("evaluations.levels.deleteConfirm names the right noun (fixed copy-paste slip)", () => {
        ...
        expect(
            tP("fr")("evaluations.levels.deleteConfirm", { name: "X" })
        ).toContain("le niveau d'évaluation");
        expect(
            tP("fr")("evaluations.levels.deleteConfirm", { name: "X" })
    ...
    test.each(["fr", "en"])(
        ...
        (lng) => {
            const v = tP(lng)("rooms.localisations.deleteConfirm", {
                name: "Zephyr",
            });
            expect(v).toContain("Zephyr");
            expect(v).not.toContain("{");
...
describe("SchoolAvailabilities", () => {
    const props = {
        planningId: 7,
        authToken: "tok",
        seasons: [{ id: 1, start: "2026-09-01", is_current: true }],
    };

    test.each(["fr", "en"])(
        "<h4> heading + <p> hint are translated in %s",
        async (lng) => {
            ...
            expect(
                screen.getByRole("heading", {
                    name: tP(lng)("plannings.schoolAvailabilities.heading"),
                })
            ).toBeInTheDocument();
            expect(
...
describe("TeacherAvailabilities", () => {
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(
                screen.getByRole("heading", {
                    name: tP(lng)("plannings.teacherAvailabilities.heading"),
                })
            ).toBeInTheDocument();
            expect(
                screen.getByText(
                    tP(lng)("plannings.teacherAvailabilities.checkboxLabel")
                )
            ).toBeInTheDocument();
            expect(
...
describe("CancelActivityParameters", () => {
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(
                screen.getByRole("heading", {
                    name: tP(lng)("plannings.cancelActivity.heading"),
                })
            ).toBeInTheDocument();
            expect(
    ...
    test.each(["fr", "en"])(
        "onSubmit success / error swal titles in %s",
        async (lng) => {
            await i18n.changeLanguage(lng);
            render(<CancelActivityParameters />);
...
describe("PlanningDisplayParameters", () => {
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(
                screen.getByRole("heading", {
                    name: tP(lng)("plannings.displayParams.heading"),
                })
            ).toBeInTheDocument();
            for (const key of [
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(swal.fire.mock.calls[0][0].title).toBe(
                tP(lng)("plannings.displayParams.saveSuccess")
            );
            ...
            expect(swal.fire.mock.calls[0][0].title).toBe(
                tP(lng)("plannings.displayParams.saveError")
            );
        }
    );
});
...
describe("EvaluationLevels", () => {
    ...
    test("default export is withTranslation-wrapped over `class extends BaseDataTable`", () => {
        ...
        expect(
            EvaluationLevels.WrappedComponent.prototype instanceof BaseDataTable
        ).toBe(true);
        expect(
            Object.getPrototypeOf(EvaluationLevels.WrappedComponent.prototype)
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(opts.title).toBe(
                t("evaluations.levels.deleteConfirm", { name: "Zephyr" })
            );
    ...
    test("explicit fr / en strings (deleteStatus swal)", async () => {
        ...
        expect(opts.title).toBe(
            "Voulez-vous vraiment supprimer le niveau d'évaluation 'Zephyr' ?"
        );
        ...
        expect(opts.title).toBe(
            "Do you really want to delete the evaluation level 'Zephyr'?"
        );
...
describe("EvaluationSlot", () => {
    async function renderSlot() {
        let utils;
        await act(async () => {
            utils = render(<EvaluationSlot />);
        });
        return utils;
    }

    test.each(["fr", "en"])(
        "<label> + submit value are translated in %s",
        async (lng) => {
            await i18n.changeLanguage(lng);
            await renderSlot();
    ...
    test.each(["fr", "en"])(
        "evaluations.slot.requiredError key resolves in %s",
        (lng) => {
        ...
        async (lng) => {
            ...
            expect(
                await screen.findByText(
                    tP(lng)("evaluations.slot.requiredError")
                )
            ).toBeInTheDocument();
        }
    );
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            const { container } = await renderSlot();
            fireEvent.input(
                container.querySelector('input[name="sessionHour"]'),
                {
                    target: { value: "30" },
                }
            );
...
describe("Localisations", () => {
    test("default export is withTranslation-wrapped over a plain React.Component", () => {
        ...
        expect(
            Localisations.WrappedComponent.prototype instanceof React.Component
        ).toBe(true);
        expect(
            Object.getPrototypeOf(Localisations.WrappedComponent.prototype)
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(got.join("|")).not.toMatch(
                /rooms\.|shared\.|translation missing/i
            );
        }
    );
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            const opts = swal.fire.mock.calls[0][0];
            expect(opts.title).toBe(
                t("rooms.localisations.deleteConfirm", { name: "Zephyr" })
            );
            expect(opts.title).toContain("Zephyr");
            expect(opts.title).not.toContain("{");
            expect(opts.cancelButtonText).toBe(t("shared.deleteConfirmNo"));
    ...
    test("explicit fr / en strings (deleteStatus swal)", async () => {
        ...
        let opts = swal.fire.mock.calls[0][0];
        expect(opts.title).toBe(
            "Voulez-vous vraiment supprimer la localisation 'Zephyr' ?"
        );
        expect(opts.cancelButtonText).toBe("non");
        expect(opts.confirmButtonText).toBe("oui");
        ...
        opts = swal.fire.mock.calls[0][0];
        expect(opts.title).toBe(
            "Do you really want to delete the location 'Zephyr'?"
        );
        expect(opts.cancelButtonText).toBe("no");
        expect(opts.confirmButtonText).toBe("yes");
    });
...
describe("LocalizationParameters (lot-E i18n additions)", () => {
    ...
    test.each(["fr", "en"])(
        "loading placeholder is common:loading in %s",
        async (lng) => {
            await i18n.changeLanguage(lng);
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            render(<LocalizationParameters />);
            act(() => {
                apiState.lastSuccess(OK_DATA);
            });

            expect(
                screen.getByRole("heading", {
                    name: tP(lng)("localization.availableHeading"),
                })
            ).toBeInTheDocument();
            expect(
                screen.getByRole("heading", {
                    name: tP(lng)("localization.defaultHeading"),
                })
            ).toBeInTheDocument();
            expect(
                screen.getByText(tP(lng)("localization.availableHint"))
            ).toBeInTheDocument();
            expect(
                screen.getByText(tP(lng)("localization.defaultHint"))
            ).toBeInTheDocument();
            expect(
    ...
    test.each(["fr", "en"])(
        ...
        async (lng) => {
            ...
            expect(swal.fire).toHaveBeenCalledTimes(1);
            expect(swal.fire.mock.calls[0][0].title).toBe(
                tP(lng)("localization.defaultMustBeAvailable")
            );
        }
    );
...
describe("PricingCategoriesEdit", () => {
    ...
    test.each(["fr", "en"])(
        "is_pack Cell renders shared.yes / shared.no in %s",
        async (lng) => {
            await i18n.changeLanguage(lng);

            globalThis.__packValue = true;
            const { unmount } = render(<PricingCategoriesEdit />);
            expect(screen.getByTestId("bdt-col-cell").textContent).toBe(
                tP(lng)("shared.yes")
            );
            unmount();
    ...
    test("explicit fr / en column headers + create button", async () => {
        ...
        expect(
            screen.getAllByTestId("bdt-col-header").map((e) => e.textContent)
        ).toEqual(HEADERS.fr);
        expect(screen.getByRole("button").textContent).toContain(
...
describe("PricingCategoryFormContent", () => {
    test("default export is withTranslation-wrapped over a plain React.Component", () => {
        ...
        expect(
            Object.getPrototypeOf(
                PricingCategoryFormContent.WrappedComponent.prototype
            )
        ).toBe(React.Component.prototype);
    });
    ...
    test.each(["fr", "en"])(
        "the three <Field label>s are translated in %s",
        async (lng) => {
            await i18n.changeLanguage(lng);
            renderInForm();

            const labels = screen
                .getAllByTestId("ff-label")
                .map((e) => e.textContent);
            expect(labels).toContain(
                tP(lng)("activities.pricing.categoryName")
            );
            expect(labels).toContain(
                tP(lng)("activities.pricing.lessonsCount")
            );
