...
export default function ConsentDocumentsList() {
    ...
    function updateDocument(document, file, fileHasChanged) {
        ...
        if (fileHasChanged) formData.append("attached_file", file);
    ...
    function handleSubmit(document, file, fileHasChanged) {
        if (document.id) updateDocument(document, file, fileHasChanged);
        else createDocument(document, file, true);
    }
    ...
    function handleMoveUp(docId) {
        api.set()
            ...
            .post(`/consent_documents/move_up`, { id: docId });
    }
    ...
    function handleMoveDown(docId) {
        api.set()
            ...
            .post(`/consent_documents/move_down`, { id: docId });
    }
    ...
    return (
        <Fragment>
            ...
            {documents.sort(compareIndices).map((doc) => {
                return (
                    <DocumentItem
                        document={doc}
                        key={doc.id}
...
function DocumentItem({
    ...
    return document ? (
        ...
        >
            ...
            >
                <div className="text-left">{document.title}</div>
            </div>
