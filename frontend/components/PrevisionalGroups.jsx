...
const SelectLevel = ({
    ...
    return (
        ...
        >
            <option value="">
                {t("evaluation:previsionalGroups.globalLevelRequired")}
            </option>
...
class PrevisionalGroups extends Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        const groupedPrevisionalActivities = _(
            Object.entries(activitiesStudents)
        )
            .groupBy(([activityId]) => {
            ...
            .map((activitiesStudents, wday) => {
                const previsionalActivities = activitiesStudents.map(
                    ([activityId, studentsEvaluations]) => {
                        const studentIds = Object.keys(
                            studentsEvaluations
                        ).map(id => parseInt(id));
                        const activityStudents = students.filter(s =>
                        ...
                        const activityStudentsLevelsIds = activityStudents.map(
                            s =>
                                findAndGet(
                                    s.levels,
                                    s =>
                ...
                const activityList = previsionalActivities.map(pa => (
                    <PrevisionalActivity
                        ...
                        handleSelectEvaluation={id =>
                            this.handleSelectEvaluation(id)
                        }
                        handleUpdateActivity={activity =>
                ...
                return (
                    <div className="col-xs-12 col-md-4" key={wday}>
                        <h4 className="text-center bg-primary p-xs sticked">
                            {_.capitalize(
                                moment()
                                    .isoWeekday(parseInt(wday))
                                    .format("dddd")
                            )}
                        </h4>
                        {activityList}
                ...
                <QuestionnaireModal
                    ...
                    header={
                        <h1>
                            <strong>
                                {t(
                                    "evaluation:previsionalGroups.evaluationSheet"
                                )}
                            </strong>
                        </h1>
                    }
                    ...
                    answers={
                        studentEvaluation &&
                        getAnswersObject(studentEvaluation.answers)
                    }
                />
            </div>
        );
...
const DisparitiesModal = ({
    ...
    const previsionalActivities = activities.map((a, i) => (
        <li key={i} className="list-group-item">
            <div className="p-xs">
                <span className="label label-primary m-r-sm">
                    {a.group_name}
                </span>
                <span className="font-bold m-r-sm">
                    {formatIntervalHours(a.time_interval)}
                </span>
            ...
            <div className="border-top p-h-xs m-t-sm border-bottom m-b-sm">
                {a.students.map(student => (
                    <div key={student.id}>
                        {fullnameWithAge(student)}
                        {", "}
                        {displayLevel(student, a.activity_ref_id, season.id)}
                    </div>
                ))}
            </div>
    ...
    return (
        <Modal
            ...
            ariaHideApp={false}
        >
            <h1>{t("evaluation:previsionalGroups.actionRequired")}</h1>
            <h2 className="text-danger">
                {t("evaluation:previsionalGroups.assignLevelPrompt")}
            </h2>
            <ul className="list-group">{previsionalActivities}</ul>
        </Modal>
    );
};
...
function PrevisionalActivity({
    ...
    const studentsList = students.map(s => {
        ...
        if (changes[s.id]) {
            changeStyle = changes[s.id].informed
                ? "change-informed"
                : "change-uninformed";
        ...
        return (
            <button
                ...
                key={s.id}
                onClick={() =>
                    handleSelectEvaluation(studentsEvaluations[s.id])
                }
            >
                <span className={`font-bold ${pursueStyle}`}>
                    {fullname(s)}
                </span>
                <span className={`${pursueStyle}`}>{toAge(s.birthday)}</span>
    ...
    if (!isDisparate) {
        if (activity.next_season_evaluation_level_ref_id)
            ...
            activityLevel =
                ...
                ) || t("evaluation:previsionalGroups.noLevel");
    }
    ...
    return (
        <div className="ibox">
            ...
            <div className="ibox-content">
                ...
                <ul className="list-group">{studentsList}</ul>
                    ...
                    <Fragment>
                        ...
                        <div>
                            <div>
                                <label>
                                    {t(
                                        "evaluation:previsionalGroups.assignGlobalLevel"
                                    )}
                                </label>
...
export default withTranslation("evaluation")(PrevisionalGroups);
