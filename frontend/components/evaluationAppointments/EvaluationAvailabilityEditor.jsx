...
export default function EvaluationAvailabilityEditor({
    ...
    const intervalIntegrity = vals =>
        vals.start >= vals.end ? "err_interval_integrity" : "";
    ...
    const intervalOverlap = vals => {
        ...
        return availability.evaluation_appointments.find(ea => {
            const oint = {
    ...
    const WithError = ({ touched, error, children }) => (
        <div>
            {children}
            {touched && error && (
                <p className="text-danger">
                    {error.startsWith("err_") ? MESSAGES[error] : error}
                </p>
            )}
        </div>
    );
        ...
        >
            {({
                ...
                const filteredRooms = values.activity_ref_id
                    ? rooms.filter(r =>
                          r.activity_refs.find(
                              ar => ar.id === parseInt(values.activity_ref_id)
                          )
                      )
                    : [];
                ...
                return (
                    ...
                    >
                        <h3>
                            {t("evaluation:appointments.editor.validatedTitle")}
                        </h3>
                        {appointments.length > 0 ? (
                            <AppointmentsList
                                ...
                                highlightId={
                                    touchedInterval &&
                                    _.get(errors.overlap, "id")
                                }
                                appointments={
                                    availability.evaluation_appointments
                                }
                            />
                        ) : (
                            <p style={{ textAlign: "center" }}>
                                {t("evaluation:appointments.editor.noneYet")}
                            </p>
                        )}
                        <hr />
                        <div className="m-t">
                            <h3>
                                {t("evaluation:appointments.editor.newTitle")}
                            </h3>
                            <form onSubmit={handleSubmit}>
                                <div className="row">
                                    <div className="col-sm-6 form-group">
                                        <label>
                                            {t(
                                                "evaluation:appointments.editor.start"
                                            )}
                                        </label>
                                        <Field
                                    ...
                                    <div className="col-sm-6 form-group">
                                        <label>
                                            {t(
                                                "evaluation:appointments.editor.end"
                                            )}
                                        </label>
                                        <Field
                                            validate={composeValidators(
                                ...
                                {touchedInterval && errors.overlap && (
                                    <p
                                        className="text-danger"
                                        style={{ textAlign: "center" }}
                                    >
                                        {t(
                                            "evaluation:appointments.editor.overlap",
                                            {
                                                start: moment(
                                                    errors.overlap.time_interval
                                                        .start
                                                ).format("HH:mm"),
                                                end: moment(
                                                    errors.overlap.time_interval
                                ...
                                {touchedInterval && errors.interval && (
                                    <p
                                        className="text-danger"
                                        style={{ textAlign: "center" }}
                                    >
                                        {MESSAGES[errors.interval]}
                                    </p>
                                )}
                                <div className="row">
                                    <div className="col-sm-6 form-group">
                                        <label>
                                            {t(
                                                "evaluation:appointments.editor.activity"
                                            )}
                                        </label>
                                        <Field
                                            validate={required}
                                            ...
                                            {props => (
                                                <WithError {...props.meta}>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            {t(
                                                                "evaluation:appointments.editor.selectActivity"
                                                            )}
                                                        </option>
                                                        {activityRefs.map(
                                                            optionMapper()
                                    ...
                                    <div className="col-sm-6 form-group">
                                        <label>
                                            {t(
                                                "evaluation:appointments.editor.room"
                                            )}
                                        </label>
                                        <Field
                                            validate={required}
                                            ...
                                            {props => (
                                                <WithError {...props.meta}>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            {t(
                                                                "evaluation:appointments.editor.selectRoom"
                                                            )}
                                                        </option>
                                                        {filteredRooms.map(
                                                            optionMapper()
...
function AppointmentsList({ t, appointments, highlightId }) {
    ...
    return (
        <table className="table table-bordered">
            <thead>
                <tr>
                    <th>
                        {t("evaluation:appointments.editor.table.schedule")}
                    </th>
                    <th>
                        {t("evaluation:appointments.editor.table.activity")}
                    </th>
                    <th>{t("evaluation:appointments.editor.table.room")}</th>
                    <th>{t("evaluation:appointments.editor.table.student")}</th>
                </tr>
            </thead>
            <tbody>
                {appointments.map(a => {
                    ...
                    return (
                        <tr
                            key={a.id}
                            style={a.id === highlightId ? hightlightStyles : {}}
                        >
                            <td>
                            ...
                            <td>
                                {a.student
                                    ...
                                    : t(
                                          "evaluation:appointments.editor.table.noStudent"
                                      )}
                            </td>
                        </tr>
                    );
