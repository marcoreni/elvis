...
describe("day column follows the active UI language (moment locale no longer forced to fr)", () => {
    ...
    test("Filter's day options render in the active language", async () => {
        ...
        const enFilter = render(
            <>{dayColumn().Filter({ filter: null, onChange: () => {} })}</>
        );
        expect(
            enFilter.getByRole("option", { name: "Monday" })
        ).toBeInTheDocument();
        enFilter.unmount();
        enList.unmount();
        ...
        const frFilter = render(
            <>{dayColumn().Filter({ filter: null, onChange: () => {} })}</>
        );
        expect(
            frFilter.getByRole("option", { name: "Lundi" })
        ).toBeInTheDocument();
        frFilter.unmount();
        frList.unmount();
    });
...
describe("row expander (UserList / UserRow) — i18n", () => {
    ...
    test("studentLevel is set straight from the API's evaluation_level_ref string, no .label", async () => {
        ...
        expect(lastApiSuccessUrl).toMatch(
            /^\/desired_activities\/user\/\d+\/activity\/\d+\/ref\/\d+\/time\/\d+$/
        );
        act(() => {
            lastApiSuccess({
                id: 1,
                evaluation_level_ref: "Débutant",
                activity_application_id: 5,
            });
        });
