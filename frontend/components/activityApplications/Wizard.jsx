...
class Wizard extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            begin_at: moment((this.props.season || {}).start).format(
                ISO_DATE_FORMAT
            ),
    ...
    isInAgeRange(a) {
        if (a.activity_type === "child") {
            ...
            const userAge = seasonEnd.diff(
                moment(this.state.infos.birthday),
                "years"
            );
            ...
            api.post(
                ...
                if (!errors) {
                    ...
                    if (this.props.preSelectedUser.id !== undefined)
                        user = data.data
                            .filter(
                                (u) => u.id === this.props.preSelectedUser.id
                            )
                            .shift();
                    else user = data.data.shift();
    ...
    handleRemoveFormula(formulaId) {
        ...
        const newSelectedFormulaActivities = {
            ...this.state.selectedFormulaActivities,
        };
        delete newSelectedFormulaActivities[formulaId];
        this.setState({
            selectedFormulaActivities: newSelectedFormulaActivities,
        });
    ...
    handleChangeSeason(season_id) {
        ...
        this.state.infos.id &&
            api
                ...
                .error(toast.error)
                .post(`/users/${this.state.infos.id}/family_links_with_user`, {
                    season_id,
                });
    }
    ...
    handleAddPack(key, packId) {
        ...
        if (
            !newSelectedPacks[key] ||
            !_.includes(newSelectedPacks[key], packId)
        ) {
    ...
    handleUpdateInstruments(ids) {
        const instruments = this.props.instruments.filter(({ id }) =>
            ids.includes(id)
        );
        this.setState({
            infos: {
    ...
    handleSubmit() {
        ...
        const formulaActivities = Object.values(
            state.selectedFormulaActivities || {}
        ).flat();

        state.selectedFormulas = Array.isArray(this.state.selectedFormulas)
            ? this.state.selectedFormulas.map((f) =>
                  typeof f === "object" ? f.id : f
              )
            : [];

        const allSelectedActivities = Array.from(
            new Set([...individualActivities, ...formulaActivities])
        );
        ...
        api.set()
            .success((data) => {
                ...
                const title = t("wizard.submit.greeting", {
                ...
                if (data.activity_application !== null) {
                    htmltext += t("wizard.submit.applicationRegisteredHtml", {
                        id: data.activity_application.id,
                    });
                }
                ...
                Swal.fire({
                    ...
                    if (
                        this.props.newApplicationForExistingUser ||
                        !this.props.currentUserIsAdmin ||
                        data.activity_application === null
                    ) {
            ...
            .post(
                `/inscriptions${authToken ? `?auth_token=${authToken}` : ""}`,
                {
                    application: state,
                    actionType: this.props.actionType,
                    preApplicationActivityId: _.get(
    ...
    prepareForActivityChoice() {
        ...
        if (
            ...
            const cycleIds =
                this.props.preApplicationActivity.activity.activity_ref.next_cycles.map(
    ...
    handleSelectUser(user) {
        ...
        const state = {
            ...
            learnedActivities,
            intervals:
                user.planning !== undefined && user.planning.time_intervals
                    ? user.planning.time_intervals.filter(
                          (interval) =>
                              !interval.is_validated &&
                              interval.start > this.props.season.start &&
                              interval.start < this.props.season.end
                      )
                    : [],
            user: user,
        };
        ...
        const familyPayers = family
            .filter((user) => user.is_paying_for)
            .map((user) => user.id);
        state.infos.payers =
            (user_infos.is_paying
                ? [...familyPayers, user_infos.id]
                : familyPayers) || [];
    ...
    handleChangePaymentInfo(key, value) {
        ...
        let paymentTerm = paymentTerms.find(
            (term) => term.season_id == this.state.season_id
        );
    ...
    handleChangePaymentTerms(paymentScheduleOptionsId) {
        this.handleChangePaymentInfo(
            "payment_schedule_options_id",
            paymentScheduleOptionsId
        );
    }
        ...
        this.setState({
            infos: {
                ...
                family_links_with_user:
                    this.state.infos.family_links_with_user.map((user) => {
                        if (this.userEquals(user, eventUser))
                            user.is_paying_for = isEventUserSelected;
                        else user.is_paying_for = payers.includes(user.id);
    ...
    getLabelsFromSelectedActivities() {
        ...
        if (this.state.selectedFormulaActivities) {
            Object.values(this.state.selectedFormulaActivities).forEach(
                (activities) => {
        ...
        selectedActivityRefIds = selectedActivityRefIds.filter(
            (activity) => !prefsReqActivityRefIds.includes(activity)
        );

        // Retourner un tableau avec les noms des activités sélectionnées
        return selectedActivityRefIds.reduce((labels, id) => {
            const element = this.props.allActivityRefs.find(
                (activityRef) => activityRef.id === id
            );
            if (element) {
    ...
    onChangeIdentificationNumber(user, identification_number) {
        this.setState({
            infos: {
                ...this.state.infos,
                identification_number: this.userEquals(user, this.state.infos)
                    ? identification_number
                    : this.state.infos.identification_number,
                family_links_with_user:
                    this.state.infos.family_links_with_user.map((u) => {
    ...
    render() {
            ...
            return (
                <Fragment>
                    ...
                    {this.state.user.is_admin ? (
                        <a className="btn btn-primary" href={"/seasons/new"}>
                            {t("wizard.createSeason")}
                        </a>
                    ) : (
                        ""
                    )}
        ...
        const activitiesThatRequirePreferencesSelection =
            this.props.allActivityRefs
                .filter((ref) => ref.allows_timeslot_selection)
                .map((ref) => ref.id);
        ...
        const timePrefsMode =
            preferencesActivities.length > 0
                ? preferencesActivities.length < allSelectedActivities.length
                    ? PLANNING_AND_PREFERENCES_MODE
                    : PREFERENCES_MODE
                : PLANNING_MODE;

        const formulaActivitiesIds = Object.values(
            this.state.selectedFormulaActivities || {}
        ).flat();
        const refsToEvaluate = this.state.activityRefs.filter(
            (ref) =>
                ...
                (this.state.selectedActivities.includes(ref.id) ||
                    formulaActivitiesIds.includes(ref.id)) &&
        ...
        const activityChoiceActionTypes = [
            PRE_APPLICATION_ACTIONS.PURSUE_CHILDHOOD,
            PRE_APPLICATION_ACTIONS.NEW,
            PRE_APPLICATION_ACTIONS.RENEW,
        ];

        const steps = [
            !this.props.preApplicationActivity &&
                this.props.preSelectedUser &&
                this.props.preSelectedUser.id === this.props.user.id && {
                    name: t("wizard.steps.member"),
                    component: this.props.currentUserIsAdmin ? (
                        <UserSearch
                ...
                this.props.formulas.length > 0 && {
                    ...
                    component: (
                        <WrappedFormulaChoice
                            ...
                            handleRemoveFormula={(id) =>
                                this.handleRemoveFormula(id)
                            }
                            handleUpdateFormulaActivities={(id, activities) =>
                                this.handleUpdateFormulaActivities(
                                    id,
                                    activities
                                )
                            }
                    ...
                    component: (
                        <WrappedActivityChoice
                            ...
                            activityRefsChildhood={
                                this.state.activityRefsChildhood
                            }
                            ...
                            handleAddActivity={(id) =>
                                this.handleAddActivity(id)
                            }
                            handleRemoveActivity={(id) =>
                                this.handleRemoveActivity(id)
                            }
                            handleAddPack={(key, id) =>
                                this.handleAddPack(key, id)
                            }
                            handleRemovePack={(key, id) =>
                                this.handleRemovePack(key, id)
                            }
                            selectedFormulas={this.state.selectedFormulas}
                            formulas={this.props.formulas}
                            selectedFormulaActivities={
            ...
            this.props.allActivityRefs.find(
                (ar) =>
                    ar.is_work_group &&
                    (this.state.selectedActivities.includes(ar.id) ||
                        Object.values(
                            this.state.selectedFormulaActivities || {}
                        )
                            .flat()
                ...
                component: (
                    <div>
                        ...
                        <Select
                            defaultValue={_.map(
                                this.state.infos.instruments,
                                ({ id: value, label }) => ({ label, value })
                            )}
                            options={this.props.instruments.map((i) => ({
                                label: i.label,
                                value: i.id,
                            }))}
                            onChange={(values) =>
                                this.handleUpdateInstruments(
                ...
                this.state.practicedInstruments.length && {
                    ...
                    component: (
                        <EvaluationIntervalChoice
                            handleUpdateSelectedEvaluationIntervals={(
                                interval
                            ) =>
            ...
            ((this.props.availPaymentScheduleOptions &&
                this.props.availPaymentScheduleOptions.length > 0) ||
                ...
                component: (
                    <WrappedPayerPaymentTerms
                        ...
                        user={{
                            ...
                            identification_number:
                                this.state.infos.identification_number,
                            birthday: this.state.infos.birthday,
                        }}
                        family={this.state.infos.family_links_with_user}
                        initialSelectedPayers={this.state.infos.payers || []}
                        paymentTerms={
                            (this.state.infos.payer_payment_terms || []).find(
                                (pt) => pt.season_id === this.state.season.id
                            ) || {}
                        }
                        collection={
                            (this.state.infos.payer_payment_terms || []).find(
                        ...
                        availPaymentScheduleOptions={
                            this.state.availPaymentScheduleOptions
                        }
                        availPaymentMethods={this.state.availPaymentMethods}
                        paymentStepDisplayText={
                            this.props.paymentStepDisplayText
                        }
                        onChangePaymentTerms={this.handleChangePaymentTerms.bind(
                            this
                        )}
                        onChangeDayForCollection={this.handleChangeDayForCollection.bind(
                            this
                        )}
                        onChangePaymentMethod={this.handleChangePaymentMethod.bind(
                            this
                        )}
                        ...
                        onChangeIdentificationNumber={this.onChangeIdentificationNumber.bind(
                            this
                        )}
                    />
                ),
...

            {
                ...
                component: (
                    <Validation
                        ...
                        packs={this.props.packs}
                        paymentTerms={
                            this.state.infos.payer_payment_terms || []
                        }
                        availPaymentScheduleOptions={
                            this.state.availPaymentScheduleOptions
                        }
        ...
        return (
            <div
                className="padding-page application-form"
                style={{ width: "85%" }}
            >
                <div className="d-md-inline-flex justify-content-between w-100"></div>
                <div
                    style={{
                        marginBottom: "35px",
                        marginTop: "45px",
                ...
                >
                    <h1 style={{ color: "#00334A" }}>
                        {t("wizard.newApplicationTitle")}
                    </h1>
                    <h3 style={{ color: "#8AA4B1" }}>
                        {t("wizard.applicationSubtitle.full", {
                            activityPart:
                                !this.state.skipActivityChoice &&
                                activityChoiceActionTypes.includes(
                                    this.props.actionType
                                )
                                    ? t(
                                          "wizard.applicationSubtitle.allActivities"
                                      )
                                    : t(
                                          "wizard.applicationSubtitle.oneActivity",
                                          {
                                              activity:
                                                  this.props.allActivityRefs.find(
                                                      (ar) =>
