...
export default function PaymentScheduleOptionForm({
    ...
    if (paymentScheduleOption) {
        if (
            !PAYMENT_SCHEDULE_OPTIONS_PAYMENTS_NUMBERS.map(
                (opt) => opt.nb
            ).includes(paymentScheduleOption.payments_number)
        ) {
            paymentScheduleOption.other_payments_number =
                paymentScheduleOption.payments_number;
            paymentScheduleOption.payments_number = 0;
        }
    }

    const [statedPaymentScheduleOption, setStatedPaymentScheduleOption] =
        useState(paymentScheduleOption || {});

    const onMonthSelected = (selected) => {
        const effectivePaymentsNumber =
            statedPaymentScheduleOption.payments_number === 0
        ...
        if (selected.length <= effectivePaymentsNumber) {
            const newStatedPaymentScheduleOption = {
    ...
    const onDayForCollectionAdd = () => {
        swal.fire({
            ...
            if (result.value) {
                ...
                newStatedPaymentScheduleOption.available_payments_days =
                    newStatedPaymentScheduleOption.available_payments_days ||
                    [];
                newStatedPaymentScheduleOption.available_payments_days.push(
    ...
    const submit = (e) => {
        if (
            (statedPaymentScheduleOption.available_payments_days || [])
                .length === 0
        ) {
        ...
        if (
            (statedPaymentScheduleOption.payments_months || []).length !==
            (statedPaymentScheduleOption.payments_number ||
                parseInt(statedPaymentScheduleOption.other_payments_number))
        ) {
        ...
        const valueToSend = {
            payment_schedule_option: {
                ...
                payments_number:
                    statedPaymentScheduleOption.payments_number === 0
                        ? statedPaymentScheduleOption.other_payments_number
                        : statedPaymentScheduleOption.payments_number,
    ...
    return (
        <Fragment>
            <div className="row wrapper border-bottom white-bg page-heading m-b-md">
                <h2>
                    {statedPaymentScheduleOption.id
                        ? t("terms.optionForm.editTitle", {
                              label: paymentScheduleOption.label,
                          })
                        : t("terms.optionForm.addTitle")}
                </h2>
            </div>
            ...
            >
                <div className="row mb-3">
                    <div className={"col-md-12"}>
                        ...
                        <input
                            ...
                            required
                            onChange={(e) =>
                                setStatedPaymentScheduleOption({
                                    ...statedPaymentScheduleOption,
                ...
                <div className="row mb-3">
                    <div className={"col-md-12"}>
                        <h4>
                            {t("terms.optionForm.numberOfDues")}{" "}
                            <span className={"text-danger"}>*</span>
                        </h4>
                        ...
                        >
                            ...
                            {PAYMENT_SCHEDULE_OPTIONS_PAYMENTS_NUMBERS.map(
                                (opt) => (
                                    <option key={opt.nb} value={opt.nb}>
                                        {opt.label}
                                    </option>
                                )
                            )}
                            <option key={0} value={0}>
                                {t("terms.optionForm.other")}
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div className="row mb-5">
                    <div className={"col-md-12"}>
                        ...
                        <small>
                            {t("terms.optionForm.associatedPricingHelp")}
                        </small>

                        <select
                            className="custom-select form-control"
                            value={
                                statedPaymentScheduleOption.pricing_category_id
                            }
                            onChange={(e) =>
                                setStatedPaymentScheduleOption({
                                    ...statedPaymentScheduleOption,
                                    pricing_category_id: parseInt(
                                        e.target.value
                                    ),
                                })
                            }
                        >
                            <option key={0} value={null}>
                                {t("terms.optionForm.none")}
                            </option>
                            {pricingCategories.map((pricingCategory) => (
                                <option
                ...
                <div className="row">
                    <div className={"col-md-12"}>
                        ...
                        <ToggleButtonGroup
                            ...
                            selected={
                                statedPaymentScheduleOption.payments_months ||
                                []
                            }
                            childrenContent={MONTHS.map((month, i) =>
                                renderMonthInGroup(month)
                            )}
                            buttonStyles={{
                                width: "100px",
                                height: "75px",
                ...
                <div className={"row mt-5"}>
                    ...
                    <div className={"col-md-12 mt-3 text-right"}>
                        <button
                            ...
                            onClick={onDayForCollectionAdd}
                            type={"button"}
                        >
                            <i className={"fa fa-plus"} />{" "}
                            {t("terms.optionForm.addDayTitle")}
                        </button>
                    </div>
                </div>
                        ...
                        statedPaymentScheduleOption.available_payments_days
                            ...
                            .map((day, i) => (
                                <div key={i} className={"col-sm-12"}>
                                    <DayForCollectionItem
                                        day={day}
                                        onDelete={(day) =>
                                            setStatedPaymentScheduleOption({
                                                ...statedPaymentScheduleOption,
                                                available_payments_days:
                                                    statedPaymentScheduleOption.available_payments_days.filter(
                                                        (d) => d !== day
                                                    ),
                                            })
                                        }
...
function DayForCollectionItem({ day, onDelete }) {
    return (
        <div className="row bg-white mx-0 my-3">
