...
export default function upcomingActivityInstancesList(props) {
    ...
    const fetchData = () => {
        api.set()
            ...
            .success(res => {
                ...
                setActivities(
                    sortActivitiesByMonth(
                        minimalDisplay ? futureActivity.slice(0, 4) : res
                    )
                );
    ...
    function sortActivitiesByDate(activities) {
        return activities.sort(
            (a, b) =>
                moment(a.time_interval.start) - moment(b.time_interval.start)
        );
    }
    ...
    if (loading)
        return (
            <Fragment>
                {t("activityApplications:packs.upcoming.loading")}
            </Fragment>
        );
                ...
                >
                    <div className="text-center my-auto">
                        <p
                            ...
                            style={{ color: "#00334A" }}
                        >
                            {t("activityApplications:packs.upcoming.noneTitle")}
                        </p>
                        <p>
                            {" "}
                            {t("activityApplications:packs.upcoming.noneText")}
                        </p>
                        <a
                            href={`/inscriptions/new?user_id=${props.user_id}`}
                            className="btn btn-primary mt-5"
                        >
                            {t("activityApplications:packs.upcoming.enrol")}
                        </a>
                    </div>
                </div>
            </div>
