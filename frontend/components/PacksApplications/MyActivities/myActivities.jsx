...
export default function myActivities() {
    ...
    function isRegistrationOpen() {
        ...
        const opening_date_for_new_applications = new Date(
            ss.opening_date_for_new_applications
        );
        const closing_date_for_applications = new Date(
            ss.closing_date_for_applications
        );
        const isCurrentSeasonOpen =
            now >= opening_date_for_new_applications &&
            now < closing_date_for_applications;
        ...
        const next_opening_date_for_new_applications = new Date(
            ns.opening_date_for_new_applications
        );
        const next_closing_date_for_new_applications = new Date(
            ns.closing_date_for_applications
        );
        const isNextSeasonOpen =
            now >= next_opening_date_for_new_applications &&
            now < next_closing_date_for_new_applications;
    ...
    function isActivityReinscriptible(activity) {
        if (activity.pre_application_activity !== undefined) {
            return (
                !activity.pre_application_activity.status && allowPreApplication
            );
        }
    ...
    function canReRegister() {
        ...
        const fromRegular = (regularActivities || []).some(
            isActivityReinscriptible
        );
    ...
    function canChildReRegister(familyMember) {
        return (
            familyMember.pre_application !== null &&
            familyMember.pre_application !== undefined
        );
    }
    ...
    const fetchData = async season_id => {
        await api
            ...
            .success(res => {
                ...
                setShowTeacherContacts(
                    res.config.show_teacher_contacts || false
                );
                    ...
                    <div className="row">
                        <div className="col-md-12">
                            <h1 className="title font-bold">
                                {t(
                                    "activityApplications:packs.myActivities.hello",
                    ...
                    <div className="row mt-lg-3">
                            ...
                            <div className="d-flex justify-content-between align-items-center mb-2 mx-4 mx-lg-0 flex-wrap activity-header">
                                <div className="px-lg-4">
                                    <h4 className="title font-bold">
                                        {t(
                                            "activityApplications:packs.myActivities.myActivities"
                                        )}
                                    </h4>
                                </div>
                                <div className="d-flex flex-column inscription-btn-container">
                                    ...
                                    {canAnyFamilyMemberReRegister() && (
                                        ...
                                        >
                                            {t(
                                                "activityApplications:packs.myActivities.reEnrol"
                                            )}
                                        </a>
                                    )}
                                </div>
                            ...
                            <div className="d-flex flex-wrap activity-cards mx-4 mt-3">
                                ...
                                {regularActivities.length > 0 &&
                                    regularActivities.map(
                                        (activity, index) =>
                                            isDesiredActivitySet(
                                                activity.desired_activities
                                            ) && (
                                                <RegularActivityCard
                                                    key={index}
                                                    activityApplication={
                                ...
                                {userActivities.length > 0 &&
                                    userActivities.map((pack, index) => (
                                        <ActivityCard key={index} pack={pack} />
                                    ))}
                                ...
                                {userActivities.length === 0 &&
                                    regularActivities.length === 0 && (
                                        <div className="w-100 text-center">
                                            ...
                                            <h3>
                                                {t(
                                                    "activityApplications:packs.myActivities.noCoursesForSeason"
                                                )}
                                            </h3>
                                        </div>
                                    )}
                        ...
                        <div className="col-xl-4 col-lg-12 d-flex flex-column">
                            <div className="d-flex align-items-baseline justify-content-between mt-3">
                                <h4 className="title font-bold">
                                    {t(
                                        "activityApplications:packs.myActivities.myNextCourses"
                                    )}
                                </h4>
                                <a
                                ...
                                >
                                    {t(
                                        "activityApplications:packs.myActivities.viewMyPlanning"
                                    )}{" "}
                                    &nbsp; &gt;
                                </a>
                            </div>
