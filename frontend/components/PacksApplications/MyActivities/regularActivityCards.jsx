    ...
    return (
        <div className="d-flex flex-wrap">
            {desired_activities.map((desired_activity, index) => {
                ...
                const start =
                    _.get(activity, "closest_instance_from_now.start", null) ||
                    _.get(activity, "time_interval.start", null);
                const end =
                    _.get(activity, "closest_instance_from_now.end", null) ||
                    _.get(activity, "time_interval.end", null);

                const phoneNumber =
                    showTeacherContacts &&
                    activity.teacher?.telephones?.length > 0
                        ? activity.teacher.telephones[0].number
                        : null;

                const hasEmail = showTeacherContacts && activity.teacher?.email;

                const contactInfoCount =
                    (phoneNumber ? 1 : 0) + (hasEmail ? 1 : 0);

                return (
                    <div
                        key={`desired-activity-${index}`}
                        className="card activity-card my-3 mx-3 h-100"
                    >
                        <div className="card-img-wrapper">
                            <img
                                ...
                                alt={t(
                                    "activityApplications:packs.card.cardImageAlt"
                                )}
                            />
                            <div className="card-banner-title background-green">
                                <p className="font-bold no-margin">
                                    {t(
                                        "activityApplications:packs.card.annual"
                                    )}
                                </p>
                            </div>
                        </div>
