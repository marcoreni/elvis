...
export default function UserPaymentsV2({
    seasons,
    user,
    is_current_user,
    onPayClicked,
    ...
    const savedSeasonId = parseInt(
        localStorage.getItem(LOCAL_STORAGE_KEY_SEASON)
    );

    const [season, setSeason] = useState(
        seasons.find((s) => s.id == savedSeasonId) ||
            _.sortBy(seasons, "start").pop()
    );
        ...
        api.set()
            ...
            .error(() => {
                ...
                swal.fire({
                    ...
                    text: t("terms.v2.prorataUpdateError"),
                    icon: "error",
                });
            })
    ...
    return (
        <Fragment>
            <div className="row wrapper border-bottom white-bg page-heading m-b-md">
                <h2>
                    {is_current_user
                        ? t("terms.v2.myPayments")
                        : t("terms.v2.paymentsOf", { name: user.full_name })}
                </h2>
            </div>
                                    ...
                                    <table className="table table-borderless m-b-md table-hover">
                                        <thead>
                                            <tr>
                                                <th>
                                                    {t(
                                                        "terms.v2.colActivities"
                                                    )}
                                                </th>
                                                <th>
                                                    {t("terms.v2.colStudent")}
                                                </th>
                                                <th>
                                                    {t("terms.v2.colProrata")}
                                                </th>
                                                <th>
                                                    {t("terms.v2.colAmount")}
                                                </th>
                                            </tr>
                                        </thead>
                        ...
                        <div className="ibox">
                            ...
                            <div className="ibox-content p-4">
                                <div className="alert alert-info p-2 px-sm-3 py-sm-4">
                                    {t("terms.v2.preferredDayInfo1", {
                                        day: paymentTerms.day_for_collection,
                                    })}{" "}
                                    <br />
                                            ...
                                            <tr key={d.id}>
                                                ...
                                                <td>
                                                    <span
                                                        ...
                                                        className={`d-block text-center text-white p-2 px-sm-4 ${d.status === 0 ? "bg-green" : d.status === 1 ? "bg-warning" : "bg-danger"}`}
                                                    >
                                                        {d.status === 0
                                                            ? t(
                                                                  "terms.v2.statusValidated"
                                                              )
                                                            : d.status === 1
                                                              ? t(
                                                                    "terms.v2.statusUpcoming"
                                                                )
                                                              : t(
                                                                    "terms.v2.statusLate"
                                                                )}
