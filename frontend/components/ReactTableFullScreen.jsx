    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.handle.active !== this.props.handle.active) {
            ...
            if (this.props.handle.active) {
                ...
                const index = this.state.classNames.indexOf(
                    "fullscreen-enabled"
                );
...
class ReactTableFullScreen extends React.Component {
    ...
    componentDidMount() {
        const handleChange = () => {
            this.setState({
                active: fscreen.fullscreenElement === this.node.current,
            });
        };

        fscreen.addEventListener("fullscreenchange", handleChange, false);
        window.addEventListener(
            `reactTableFullscreen${this.state.tableName}Change`,
            this.enter,
            false
        );
    }
                ...
                {this.state.showDefaultFullScreenButton ? (
                    <button
                        ...
                        data-tippy-content={t(
                            "common:baseDataTable.fullScreenTooltip"
                        )}
                    >
...
export default withTranslation("common")(ReactTableFullScreen);
